defmodule Elektrine.ActivityPub.SigningKey do
  @moduledoc """
  Manages cryptographic signing keys for ActivityPub federation.
  ...
  # Minimum seconds between key refetch attempts
  ...
  @doc """
  Changeset for creating/updating a remote signing key.
  """
  ...
  @doc """
  Changeset for creating a local user's signing key.
  """
  ...
  @doc """
  Gets a signing key by key_id, fetching from remote if not found.
  ...
  @doc """
  Refreshes a signing key if it's old enough.
  ...
  @doc """
  Fetches a remote key by key_id.
  ...
  def fetch_remote_key(key_id) do
    ...
    # Extract actor URI from key_id (e.g., "https://example.com/users/alice#main-key" -> "https://example.com/users/alice")
  ...
  @doc """
  Creates or updates a signing key for a local user.
  """
  ...
  @doc """
  Gets the signing key for a local user.
  """
  def get_for_user(user_id) do
    from(sk in __MODULE__, where: sk.user_id == ^user_id)
    |> Repo.one()
  end

  @doc """
  Gets the signing key for a remote actor.
  """
  def get_for_remote_actor(remote_actor_id) do
    from(sk in __MODULE__, where: sk.remote_actor_id == ^remote_actor_id)
    |> Repo.one()
  end

  @doc """
  Generates the key_id for a local user.
  ...
  @doc """
  Extracts the actor URI from a key_id.
  """
  ...
  def public_key_decoded(_), do: {:error, :no_key}

  @doc """
  Decodes the private key from PEM format.
  Returns {:ok, decoded_key} or {:error, reason}.
  """
  ...
  def private_key_decoded(_), do: {:error, :no_key}
  ...
  def generate_key_pair do
    ...
    # Extract and encode public key in PKCS#8/SubjectPublicKeyInfo format
  ...
  @doc """
  Signs a string with a signing key.
  ...
  @doc """
  Verifies a signature with a signing key.
    ...
    case public_key_decoded(key) do
      {:ok, public_key} ->
        case Base.decode64(signature) do
          {:ok, decoded_sig} ->
            :public_key.verify(data, :sha256, decoded_sig, public_key)
