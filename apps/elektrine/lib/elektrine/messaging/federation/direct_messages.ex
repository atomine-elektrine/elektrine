defmodule Elektrine.Messaging.Federation.DirectMessages do
  ...
  def apply_event("dm.message.create", data, remote_domain, context)
    ...
    with %{} = dm_payload <- data["dm"],
         ...
         {:ok, recipient_user} <- call(context, :resolve_local_dm_recipient, [dm_payload["recipient"]]),
         {:ok, remote_sender} <-
  ...
  def apply_event(_event_type, _data, _remote_domain, _context), do: {:error, :unhandled_event_type}
