defmodule Elektrine.Profiles.ProfileLink do
  @moduledoc """
  Schema for social media and website links displayed on user profiles.
  Supports multiple platforms with custom icons, tracking, and URL safety validation.
  """
  use Ecto.Schema
  ...
  schema "profile_links" do
    ...
    field :icon, :string, default: "hero-link"
    ...
    field :display_style, :string
    # "none", "glow", "pulse", "border", "shine", or nil (use profile default)
    field :highlight_effect, :string
  ...
  def changeset(link, attrs) do
    ...
    attrs =
      for {key, value} <- attrs, into: %{} do
        {to_existing_atom_safe(key), value}
      end
  ...
  # Safely convert keys to atoms, only for known schema fields
  # Unknown keys are kept as strings to prevent atom exhaustion DoS
  defp to_existing_atom_safe(key) when is_atom(key), do: key
  ...
  @doc """
  Gets the icon for a social platform using platform logos.
  """
  def get_platform_icon(platform) do
    case platform do
