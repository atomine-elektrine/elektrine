defmodule Mix.Tasks.Arblarg.Conformance do
  use Mix.Task

  alias Elektrine.Messaging.ArblargProfiles

  @shortdoc "Runs Arblarg conformance suites and writes badge artifacts"

  @moduledoc """
  Runs Arblarg conformance test suites and writes profile + extension badge artifacts.
  ...
  defp run_test_suite(path) do
    Mix.shell().info("Running Arblarg conformance suite: #{path}")
  ...
  defp write_badge(file_name, payload) do
    ...
    Mix.shell().info("Wrote Arblarg badge artifact: #{badge_file}")
  end
  ...
  defp profile_badge_file_name(profile_id) do
    case to_string(profile_id) do
      "arblarg-core/1.0" -> "arblarg-core-1.0.json"
      "arblarg-community/1.0" -> "arblarg-community-1.0.json"
