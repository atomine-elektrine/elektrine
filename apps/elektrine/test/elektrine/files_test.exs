defmodule Elektrine.FilesTest do
  ...
  setup do
    ...
    tmp_dir =
      Path.join(System.tmp_dir!(), "elektrine-files-#{System.unique_integer([:positive])}")
  ...
  test "upload_file stores file metadata and content", %{user: user} do
    ...
    assert {:ok, file} = Files.upload_file(user, "reports/2026", upload)
    ...
    assert {:ok, "quarterly numbers"} = Files.read_file(file)

    [stored] = Files.list_files(user.id)
    assert stored.id == file.id
    assert Files.storage_used(user.id) == byte_size("quarterly numbers")
  end
  ...
  test "create_share and revoke_share manage public links", %{user: user} do
    ...
    assert {:ok, share} = Files.create_share(user.id, file.id, %{expires_in: "7d"})
    share_id = share.id
    assert %Files.FileShare{id: ^share_id} = Files.get_active_share(share.token)
    assert share.expires_at

    assert {:ok, updated_share} = Files.increment_share_download_count(share)
    assert updated_share.download_count == 1

    assert {:ok, _revoked} = Files.revoke_share(user.id, share.id)
    assert is_nil(Files.get_active_share(share.token))
  end

  test "list_folder returns immediate folders and files", %{user: user} do
    assert {:ok, _} =
             Files.upload_file(user, "projects/alpha", temp_upload("readme.txt", "alpha"))

    assert {:ok, _} = Files.upload_file(user, "projects/beta", temp_upload("notes.txt", "beta"))
    assert {:ok, root_file} = Files.upload_file(user, "", temp_upload("todo.txt", "root"))

    assert {:ok, root_view} = Files.list_folder(user.id, "")
  ...
  test "can create, rename, and move folders", %{user: user} do
    assert {:ok, folder} = Files.create_folder(user.id, "workspace/drafts")
    assert folder.path == "workspace/drafts"

    assert {:ok, moved_path} = Files.rename_folder(user.id, "workspace/drafts", "plans")
    assert moved_path == "workspace/plans"

    assert {:ok, final_path} = Files.move_folder(user.id, "workspace/plans", "archive/2026")
    assert final_path == "archive/2026/plans"

    assert {:ok, view} = Files.list_folder(user.id, "archive/2026")
    assert Enum.map(view.folders, & &1.path) == ["archive/2026/plans"]
  end

  test "supports search and sort in folder views", %{user: user} do
    assert {:ok, _} = Files.upload_file(user, "", temp_upload("zeta.txt", "12345"))
    assert {:ok, _} = Files.upload_file(user, "", temp_upload("alpha.txt", "1"))

    assert {:ok, search_view} = Files.list_folder(user.id, "", %{q: "alp"})
    assert Enum.map(search_view.files, & &1.original_filename) == ["alpha.txt"]

    assert {:ok, sorted_view} = Files.list_folder(user.id, "", %{sort: "name_desc"})
  ...
  test "expired shares are no longer active", %{user: user} do
    assert {:ok, file} = Files.upload_file(user, "", temp_upload("shared.txt", "expiring"))
    assert {:ok, share} = Files.create_share(user.id, file.id, %{expires_in: "1d"})
    ...
    assert is_nil(Files.get_active_share(expired_share.token))
  end

  test "share links can be password protected and inline viewable", %{user: user} do
    assert {:ok, file} = Files.upload_file(user, "", temp_upload("preview.txt", "hello world"))

    assert {:ok, share} =
             Files.create_share(user.id, file.id, %{
               access_level: "view",
    ...
    assert Files.share_requires_password?(share)
    assert Files.verify_share_password(share, "secret-pass")
    refute Files.verify_share_password(share, "wrong-pass")
  ...
  test "uploading the same path replaces the existing file", %{user: user} do
    ...
    assert Files.storage_used(user.id) == byte_size("two two")
    assert {:ok, "two two"} = Files.read_file(second)

    refute File.exists?(Path.join(uploads_dir(), first.storage_key))
  end

  test "can rename, move, and bulk delete files", %{user: user} do
    assert {:ok, file} = Files.upload_file(user, "drafts", temp_upload("notes.txt", "hello"))
    assert {:ok, _folder} = Files.create_folder(user.id, "archive")

    assert {:ok, renamed} = Files.rename_file(user.id, file.id, "ideas.txt")
    assert renamed.path == "drafts/ideas.txt"

    assert {:ok, moved} = Files.move_file(user.id, renamed.id, "archive")
    assert moved.path == "archive/ideas.txt"

    assert :ok = Files.bulk_delete(user.id, ["file:#{moved.id}"])
    assert is_nil(Files.get_file(user.id, moved.id))
  end

  defp user_fixture do
    {:ok, user} =
      Accounts.create_user(%{
        username: "files#{System.unique_integer([:positive])}",
        password: "Test123456!",
