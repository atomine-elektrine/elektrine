defmodule ElektrineEmailWeb.ContactsLive.Operations.ContactOperations do
  @moduledoc """
  ...
  def handle_contact_event("new_contact", _params, socket) do
    {:noreply,
     ...
     |> assign(:contact_changeset, Contact.changeset(struct(Contact), %{}))}
  end
  ...
  def handle_contact_event("validate_contact", %{"contact" => params}, socket) do
    contact = socket.assigns.editing_contact || struct(Contact)
  ...
  def handle_contact_event("new_group", _params, socket) do
    {:noreply,
     ...
     |> assign(:group_changeset, ContactGroup.changeset(struct(ContactGroup), %{}))}
  end
  ...
  def handle_contact_event("validate_group", %{"contact_group" => params}, socket) do
    group = socket.assigns.editing_group || struct(ContactGroup)
