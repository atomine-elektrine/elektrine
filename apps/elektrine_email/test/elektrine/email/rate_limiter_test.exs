defmodule Elektrine.Email.RateLimiterTest do
  ...
  describe "rate limiting" do
    ...
    test "check_rate_limit does not enforce the legacy daily count", %{user: user} do
      ...
      timestamp = now - 3700
      attempts = List.duplicate(timestamp, 10)

      case :ets.lookup(:email_rate_limiter, user.id) do
        [] ->
          :ets.insert(:email_rate_limiter, {user.id, attempts})
    ...
    test "get_rate_limit_status returns correct status for new user", %{user: user} do
      ...
      # New TL0 user on day 1 has limits of {1, 5, 10, 3}
      assert status.daily.sent == 0
      assert status.daily.limit == 10
      assert status.daily.remaining == 10
    ...
    test "get_rate_limit_status tracks recorded attempts", %{user: user} do
      ...
      assert status.daily.limit == 10
      assert status.daily.remaining == 8
    end
  end
  ...
  describe "tier selection" do
    test "new TL0 user gets day_1 tier" do
      ...
      assert status.tier == :day_1
      # day_1 limits: {1, 5, 10, 3}
      assert status.attempts[86_400].limit == 10
