defmodule ElektrinePasswordManagerWeb.VaultLive do
  ...
  def render(assigns) do
    ...
    <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 pb-2">
      <nav aria-label="Vault navigation" class="mb-4 sticky top-16 z-40">
        <div class="rounded-2xl border border-base-300 bg-base-100/95 shadow-sm backdrop-blur-sm">
          <div class="flex items-center gap-2 overflow-x-auto px-3 py-3">
            <a href="/overview" class="btn btn-ghost btn-sm">Overview</a>
            <a href="/account/password-manager" class="btn btn-sm btn-primary">Vault</a>
          </div>
        </div>
      </nav>
                    ...
                    <tbody>
                      <%= for entry <- @entries do %>
                        <tr
                          ...
                          data-encrypted-password={Payloads.encode_payload(entry.encrypted_password)}
