defmodule Elektrine.VPNTest do
  ...
  describe "generate_config_file/1" do
    ...
    test "builds Shadowsocks snapshots from env-style string port ranges" do
      {:ok, server} =
        VPN.create_server(%{
          name: "Env SS",
          protocol: "shadowsocks",
      ...
      assert snapshot.server.port_range == %{"start" => 8388, "end" => 8488, "size" => 101}
    end
  end
  ...
  describe "ensure_self_host_server/1" do
    ...
    test "creates both wireguard and shadowsocks self-hosted servers on the same host" do
      ...
      assert wireguard.endpoint_port == 443
