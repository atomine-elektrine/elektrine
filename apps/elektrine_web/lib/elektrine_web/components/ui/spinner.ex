defmodule ElektrineWeb.Components.UI.Spinner do
  @moduledoc """
  SVG spinner components from @iconify-json/svg-spinners.
  """
  use Phoenix.Component
  import Phoenix.HTML, only: [raw: 1]

  @doc """
  Renders a rotating ring spinner.
  ...
  attr :variant, :string, default: "ring", values: ["ring", "dots", "pulse"]

  def spinner(assigns) do
    ~H"""
    <svg
      class={[@class, spinner_animation(@variant)]}
      viewBox="0 0 24 24"
      xmlns="http://www.w3.org/2000/svg"
      fill="currentColor"
      aria-hidden="true"
    >
      {raw(spinner_body(@variant))}
    </svg>
    """
  end

  # wind-toy uses CSS animation, others use SMIL
  defp spinner_animation("ring"), do: "animate-spin"
  defp spinner_animation(_), do: nil

  # wind-toy spinner (default)
  defp spinner_body("ring") do
  ...
  # pulse spinner
  defp spinner_body("pulse") do
    ~s|<circle cx="12" cy="12" r="0" fill="currentColor"><animate id="a" attributeName="r" begin="0;a.end+0.5s" calcMode="spline" dur="1.5s" keySplines=".52,.6,.25,.99" values="0;11"/><animate attributeName="opacity" begin="0;a.end+0.5s" calcMode="spline" dur="1.5s" keySplines=".52,.6,.25,.99" values="1;0"/></circle>|
  end

  defp spinner_body(_), do: spinner_body("ring")
end
