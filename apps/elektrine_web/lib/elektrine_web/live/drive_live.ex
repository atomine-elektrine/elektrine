defmodule ElektrineWeb.DriveLive do
  ...
  def handle_params(params, _uri, socket) do
    ...
    case Drive.list_folder(user.id, Map.get(params, "folder", ""), params) do
      {:ok, folder_view} ->
  ...
  def handle_event("upload_files", %{"upload" => params}, socket) do
    ...
    results =
      consume_uploaded_entries(socket, :files, fn %{path: path}, entry ->
        ...
        {:ok, Drive.upload_file(user, folder_path, upload)}
      end)
  ...
  def handle_event("filter", %{"filters" => params}, socket) do
    ...
    {:noreply, push_patch(socket, to: drive_path(folder, query, sort, filter))}
  end
  ...
  def handle_event("delete_file", %{"id" => id}, socket) do
    ...
    with {:ok, file_id} <- parse_id(id),
         :ok <- Drive.delete_file(user.id, file_id) do
      {:noreply,
       socket
       |> assign(:active_file, nil)
       |> refresh_files(user.id, socket.assigns.current_folder)
  ...
  def handle_event("rename_file", %{"rename" => %{"id" => id, "name" => name}}, socket) do
    ...
    with {:ok, file_id} <- parse_id(id),
         {:ok, _file} <- Drive.rename_file(user.id, file_id, name) do
      {:noreply,
       socket
       |> assign(:rename_target, nil)
  ...
  def handle_event(
    ...
    case Drive.rename_folder(user.id, path, name) do
      {:ok, new_path} ->
        current_folder =
  ...
  def handle_event("toggle_tree_node", %{"path" => path}, socket) do
    ...
    all_folders = Drive.list_folders(socket.assigns.current_user.id)
  ...
  def handle_event("move_file", %{"move" => %{"id" => id, "folder" => folder}}, socket) do
    ...
    with {:ok, file_id} <- parse_id(id),
         {:ok, _file} <- Drive.move_file(user.id, file_id, folder) do
      {:noreply,
       socket
       |> assign(:move_target, nil)
       |> refresh_active_file(user.id, file_id)
  ...
  def handle_event(
    ...
    case Drive.move_folder(user.id, path, folder) do
      {:ok, _new_path} ->
        {:noreply,
         socket
         |> assign(:move_target, nil)
  ...
  def handle_event("open_file", %{"id" => id}, socket) do
    ...
    case parse_id(id) do
      {:ok, file_id} ->
        case Drive.get_file(user.id, file_id) do
          nil ->
          ...
          file ->
            destination =
              if inline_viewable?(file.content_type) do
                ~p"/account/drive/#{file.id}/preview"
              else
                ~p"/account/drive/#{file.id}/download"
              end
  ...
  def handle_event("bulk_delete", _params, socket) do
    ...
    case Drive.bulk_delete(user.id, MapSet.to_list(socket.assigns.selected_items)) do
      :ok ->
        {:noreply,
         socket
         |> refresh_files(user.id, socket.assigns.current_folder)
  ...
  def handle_event("bulk_move", %{"bulk_move" => %{"folder" => folder}}, socket) do
    ...
    case Drive.bulk_move(user.id, MapSet.to_list(socket.assigns.selected_items), folder) do
      :ok ->
        {:noreply,
         socket
         |> assign(:bulk_move_open, false)
  ...
  def handle_event("drag_move_items", %{"tokens" => tokens, "folder" => folder}, socket)
    ...
    case Drive.bulk_move(user.id, tokens, folder) do
      :ok ->
        {:noreply,
         socket
         |> refresh_files(user.id, socket.assigns.current_folder)
  ...
  def handle_event("create_share", %{"share" => %{"file_id" => id} = params}, socket) do
    ...
    with {:ok, file_id} <- parse_id(id),
         {:ok, _share} <- Drive.create_share(user.id, file_id, params) do
      {:noreply,
       socket
       |> refresh_active_file(user.id, file_id)
       |> refresh_files(user.id, socket.assigns.current_folder)
  ...
  def handle_event("revoke_share", %{"id" => id}, socket) do
    ...
    with {:ok, share_id} <- parse_id(id),
         {:ok, _share} <- Drive.revoke_share(user.id, share_id) do
      {:noreply,
       socket
       |> refresh_active_file(
         user.id,
  ...
  defp refresh_files(socket, user_id, folder_path) do
    case Drive.list_folder(user_id, folder_path, %{
  ...
  defp refresh_active_file(socket, user_id, file_id) do
    active_file = Enum.find(Drive.list_files(user_id), &(&1.id == file_id))
    assign(socket, :active_file, active_file)
  end

  defp apply_folder_view(socket, folder_view, user_id) do
    all_folders = Drive.list_folders(user_id)
  ...
  defp build_folder_tree(folders, current_folder, tree_expanded_paths) do
    ...
    [%{path: "", name: "My Drive", depth: 0, has_children: true, expanded: true}] ++
      (paths
       |> Enum.map(fn path ->
         %{
           path: path,
  ...
  defp share_url(share), do: url(~p"/drive/share/#{share.token}")
  defp share_password_protected?(share), do: Drive.share_requires_password?(share)
  defp inline_viewable?(content_type), do: Drive.inline_viewable_content_type?(content_type)

  defp image_preview?(content_type),
    do: is_binary(content_type) and String.starts_with?(content_type, "image/")

  defp drive_path(folder, query, sort, filter) do
