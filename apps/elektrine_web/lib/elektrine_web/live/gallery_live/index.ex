defmodule ElektrineWeb.GalleryLive.Index do
  ...
  def mount(_params, session, socket) do
    ...
    # Allow image uploads for authenticated users
    socket =
      if user do
  ...
  def handle_event("save_gallery_photo", _params, socket) do
    ...
    if Enum.empty?(uploaded_files) do
      ...
      # Create gallery post directly with post_type: "gallery"
      case Social.create_timeline_post(
        ...
        {:ok, gallery_post} ->
          ...
          # Broadcast new gallery post (the broadcast will add it to the feed via handle_info)
        ...
        {:error, changeset} ->
          ...
          {:noreply,
           socket
           |> assign(:uploading, false)
           |> put_flash(:error, "Failed to upload photo")}
      end
    end
  end
  ...
  def handle_event("view_photo", %{"photo_id" => photo_id}, socket) do
    photo_id = if is_binary(photo_id), do: String.to_integer(photo_id), else: photo_id
  ...
  def handle_event("load-more", _params, socket) do
    # Infinite scroll triggered - load more photos
    if socket.assigns.loading_more || socket.assigns.end_of_feed do
      {:noreply, socket}
    else
      current_posts = socket.assigns.gallery_posts
      # Use timestamp for pagination since local and federated posts have different ID sequences
      before_timestamp =
        if Enum.empty?(current_posts), do: nil, else: List.last(current_posts).inserted_at
      ...
      # Filter out any posts we already have (same timestamp edge case)
  ...
  def handle_event(
    ...
    {modal_post, is_liked} =
      if params["photo_id"] do
        ...
        # Check actual database state for like status
        liked =
  ...
  # Navigate to next post in gallery (up/down arrow keys)
  ...
  def handle_event("toggle_modal_like", %{"post_id" => post_id}, socket) do
    if !socket.assigns[:current_user] do
      ...
      user_id = socket.assigns.current_user.id
      photo_id = if is_binary(post_id), do: String.to_integer(post_id), else: post_id

      # Check actual database state, not UI state
  ...
  def handle_info(:load_gallery_data, socket) do
    ...
    # Get gallery posts - messages with post_type: "gallery"
    gallery_posts = get_gallery_feed("discover", user)

    # Get user likes for gallery posts
    user_likes =
      if user do
    ...
    {user_gallery_stats, suggested_photographers, trending_tags} =
      if user do
        ...
        photographers =
          if !stats || stats.photo_count == 0, do: Task.await(photographers_task, 5000), else: []
  ...
  defp get_gallery_feed("discover", _user, opts) do
    ...
    # Federated posts with images (load all, filter in memory later)
    # Include "unlisted" visibility since many Pixelfed/Mastodon posts use it
    federated_query =
      from(m in Messaging.Message,
        where:
  ...
  defp get_gallery_feed("following", user, opts) do
    if user do
      ...
      # Get followed remote actor IDs (accepted follows only)
      ...
      # Local gallery posts from followed users
      local_query =
        from(m in Messaging.Message,
          where:
      ...
      # Federated posts with images from followed remote actors
      federated_posts =
        if Enum.empty?(following_remote_actor_ids) do
          ...
          federated_query =
            from(m in Messaging.Message,
              where:
                m.federated == true and
                  m.remote_actor_id in ^following_remote_actor_ids and
                  is_nil(m.deleted_at) and
                  is_nil(m.reply_to_id) and
                  fragment("array_length(?, 1)", m.media_urls) > 0,
              order_by: [desc: m.inserted_at],
  ...
  defp get_gallery_feed("trending", _user, opts) do
    ...
    # Get trending federated image posts
    # Include "unlisted" visibility since many Pixelfed/Mastodon posts use it
    federated_query =
      from(m in Messaging.Message,
        where:
    ...
    # Combine and sort by likes then date
    (local_posts ++ federated_posts)
  ...
  defp get_gallery_feed(_, _user, opts), do: get_gallery_feed("discover", nil, opts)

  # Override to return MapSet for compatibility with gallery logic
  ...
  # Calculate consecutive days user has uploaded
  defp calculate_upload_streak(user_id) do
    import Ecto.Query

    # Get upload dates for last 30 days
  ...
  defp count_consecutive_days([], _current_date, streak), do: streak
  ...
  # Get suggested photographers for viewers - optimized to avoid N+1
  defp get_suggested_photographers(user_id, opts) do
    import Ecto.Query
    limit = Keyword.get(opts, :limit, 5)

    # Get users who upload frequently to gallery and aren't followed
    followed_ids =
      from(f in Elektrine.Profiles.Follow,
  ...
  defp apply_gallery_filter(socket) do
    ...
    # Then apply software filter
    filtered_posts =
      filter_posts_by_software(category_filtered, socket.assigns.software_filter)

    assign(socket, :filtered_posts, filtered_posts)
  end

  # Filter posts by gallery category
  defp filter_posts_by_category(posts, "all"), do: posts
  ...
  # Normalize any timestamp to NaiveDateTime for consistent DB queries
  ...
  # Filter posts by software type using nodeinfo
