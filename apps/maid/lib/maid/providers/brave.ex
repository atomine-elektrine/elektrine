defmodule Maid.Providers.Brave do
  @moduledoc "Brave Search API provider for Maid."
  ...
  def search(query, opts) when is_binary(query) do
    ...
    with {:ok, endpoint} <- endpoint(kind),
         ...
         {:ok, payload} <- Maid.HTTP.get_json(url(endpoint, query, opts), headers(api_key), opts) do
  ...
  defp api_key(opts) do
    key = Keyword.get(opts, :api_key) || Application.get_env(:maid, :brave_api_key)
  ...
  defp normalize_kind(kind) when kind in [:web, :images, :videos, :news], do: kind
  ...
  defp normalize_kind(_kind), do: :web
