...
test_pool_size =
  case System.get_env("TEST_POOL_SIZE") || System.get_env("POOL_SIZE") do
    nil ->
      min(System.schedulers_online() * 4, 32)

    value ->
      case Integer.parse(value) do
        {int, ""} when int > 0 -> int
        _ -> min(System.schedulers_online() * 4, 32)
      end
  end
...
# Skip security check verification in tests
config :elektrine, :atomine_pow, difficulty: 0, skip_verification: true
