...
normalize_platform_modules() {
  ...
  if [[ -z "$trimmed" || "$trimmed" == "all" || "$trimmed" == "*" ]]; then
    ...
    BUILD_SLUG="all"
    NORMALIZED_MODULE_ARRAY=(chat social email nerve vpn dns uptime atomine kairo)
    return
  fi
  ...
  local candidate=""
  for candidate in chat social email nerve vpn dns uptime atomine kairo; do
  ...
  if [[ "$NORMALIZED_MODULES" == "chat,social,email,nerve,vpn,dns,uptime,atomine,kairo" ]]; then
    BUILD_SLUG="all"
  else
